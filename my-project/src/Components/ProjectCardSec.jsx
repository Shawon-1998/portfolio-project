...
const ProjectCardSec = ({ImgSrc, item1, item2, item3, projectName,link1,link2, description }) => {
    return (
        <>
            <Container>
                <div className=' card lg:h-120 h-110  pb-3 lg:w-88   text-center shadow-[0_0_10px] rounded-xl hover:scale-110 transition-all linear 0.3s '>
                    <img src={ImgSrc} className='full rounded-t-xl' alt="" />
                    <div className='flex gap-3 mt-5 justify-center pt-3  pb-5 '>
                        <h3>{item1}</h3>
                        <h3>{item2}</h3>
                        <h3>{item3}</h3>
                    </div>
                    <h2 className='lg:text-3xl text-xl font-semibold'>{projectName}</h2>
                    <p className='lg:pt-5 pt-2 pb-3 lg:pb-6'>{description}</p>
                    <Flex>
